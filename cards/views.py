...
def delivery_view(request):
    try:
        ...
        # Auto-create delivery record if missing
        if not delivery:
            delivery = CardDelivery.objects.create(
                card=card,
                address_line1=patient.address or 'Not specified',
                city='Not specified',
                state='Not specified',
                pin_code='000000',
                delivery_mobile=patient.mobile or '0000000000',
                status='processing',
            )
    ...
    current_idx = step_names.index(delivery.status) if delivery.status in step_names else 0
